<template>
    <view class="ai-chat-container">
        ...
        <scroll-view class="message-list no-scrollbar" scroll-y :scroll-top="scrollTop" :scroll-into-view="scrollTarget"
            :show-scrollbar="false" @scrolltoupper="loadMoreMessages" :style="{ height: scrollViewHeight + 'rpx' }"
        ...
        <view class="input-area" :style="{ paddingBottom: inputBottom + 'px' }">
            <van-field  placeholder-style="color: #8b8b8b; " :placeholder="inputBottom"
                autosize :value="inputMessage" :adjust-position="false" :show-confirm-bar="false" :border="false"
                :custom-style="fieldStyle" @focus="onFieldFocus" @blur="onFieldBlur" @change="onFieldChange"
...
import { APPID, WORKFLOWID, COZE_TOKEN } from "@/utils/config";
...
const fieldStyle = ref(
    // "background-color: #efefef; border-radius: 30rpx; font-size: 32rpx;"
    "background-color: #efefef;border-radius: 10rpx; padding: 10rpx;"
);
...
const initKeyboardListener = () => {
    uni.onKeyboardHeightChange((res) => {
        ...
        if (res.height > 0) {
            // 键盘弹出时，只使用键盘高度
            inputBottom.value = res.height - 75;
